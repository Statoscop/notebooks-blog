...
df <- tibble(prob_1 = numeric(101),
             cote_1 = numeric(101),
             prob_2 = numeric(101),
             cote_2 = numeric(101))


df %>% mutate(prob_1 = seq(0, 1, 0.01),
              cote_1 = prob_1/(1-prob_1)) -> df
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap = "Cotes correspondantes à chaque niveau de probabilité"}
df %>% ggplot(aes(x = prob_1, y = cote_1)) + 
  geom_line() + 
  scale_y_log10(breaks = c(0.1, 0.5, 1, 2, 10)) + 
  labs(x = "Probabilité", y = "Cote (échelle log)") + 
  theme_light()
```
