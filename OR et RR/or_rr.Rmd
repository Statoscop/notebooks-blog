...
library(dplyr)
library(ggplot2)
df <- tibble(probs = numeric(101),
             cote = numeric(101))


df %>% mutate(probs = seq(0, 1, 0.01),
              cote = probs/(1-probs)) -> df
...
df %>% ggplot(aes(x = probs, y = cote)) + 
  geom_line() + 
  scale_y_log10(breaks = c(0.1, 0.5, 1, 2, 10)) + 
  labs(title = "Cotes correspondantes à chaque niveau de probabilité",
       x = "Probabilité", y = "Cote (échelle log)") + 
  theme_light()
```
