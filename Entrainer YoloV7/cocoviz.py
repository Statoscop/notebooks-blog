...
"""
Code issu du lien suivant (et modifié à la marge) :
https://www.kaggle.com/code/ericdepotter/visualize-coco-annotations
"""
...
class CocoDataset():
    ...
    def display_categories(self):
        ...
        for image_id in self.images.keys():
            for dic in self.segmentations[image_id]:
                img_all_cats.append(dic['category_id'])

        cat_counts = Counter(img_all_cats)
            
        print('Categories:')
        print('=========')
        for sc_key, sc_val in self.super_categories.items():
            print(f"  {'super_category:' + sc_key:<40} Occurences")
            sc_counts = 0
            for cat_id in sc_val:
                print(f"    id {str(cat_id) + ':' + self.categories[cat_id]['name']:<40} {cat_counts[cat_id]}")
                sc_counts += cat_counts[cat_id] 
            print(f"       {'Total':<40} {sc_counts}")
            print('')
